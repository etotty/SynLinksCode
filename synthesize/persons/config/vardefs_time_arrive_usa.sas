...
*** BASIC INFO;
%let variable=time_arrive_usa; /* name of sas variable on input file to be imputed */
%let level=3; /* will help determine order of imputation */
%let ever_out_of_universe=1; /* =1 if &variable. is ever out of universe, =0 otherwise */
%let universe_determinants=foreign_born; /* list of variables used to determine when &variable. is in universe */
...
foreign_born=1
%mend;
%let transform_depvar=1;
/*
 =1 if &variable. should be transformed to standard normal scale prior to estimation
      and then transformed back to original scale after imputation,
 =0 otherwise
*/


*** STRATIFICATION;
%let num_lists=5; /* number of stratification lists provided from finest to coarsest */
/* next: list of numerical, categorical variables to stratify data before regression/bootstrap */
%let stratifiers1=male linked_couple linked_child hhh_flag;
%let stratifiers2=male linked_couple linked_child;
%let stratifiers3=male linked_child;
%let stratifiers4=linked_child;
%let stratifiers5=;
/*******
...
********************************************************/
if _n_=1 then put "GARY";
if _n_<100 then put time_arrive_usa;
if time_arrive_usa>.Z then time_arrive_usa=min(11,max(1,round(time_arrive_usa,1)));
if _n_<100 then put time_arrive_usa;
