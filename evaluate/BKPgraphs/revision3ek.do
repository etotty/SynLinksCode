...
global datapath ../../synthesize/couples
global data COMP
global dataname comparison_couples



*Goal: producing graphs of the distribution of relative earning within couples by education

set more off
log using `outputDir'output.log, replace

log off

use ${datapath}/${dataname}.dta, clear
*identify husband and  wife within a couple

sort personid spouse_personid
keep if personid~=. & spouse_personid~=.
...
gen myhhid = shusb_id + swife_id
egen tag=tag(myhhid)

*compute income for each individual in first year of each panel
gen earn_2009 = total_der_fica_2009

* education: 1-5, 3=some college
gen edu = .
replace edu = 0 if educ_5cat==1 | educ_5cat==2
replace edu = 1 if educ_5cat==3 | educ_5cat==4 | educ_5cat==5

keep myhhid swife_id shusb_id tag earn_* panel male edu

gen earn = .

*create income measures that are not year specific
replace earn=earn_2009

*compute income measures for husband and wife
gen rwife_earn=.
gen rhusb_earn=.
replace rwife_earn=earn if male==0
replace rhusb_earn=earn if male==1
bysort myhhid: egen wife_earn = mean(rwife_earn)
bysort myhhid: egen husb_earn = mean(rhusb_earn)

*compute relative income measures within couples
gen rel_earn=wife_earn/(wife_earn + husb_earn)


*compute education measures for husband and wife
gen rwife_edu=.
gen rhusb_edu=.
replace rwife_edu=edu if male==0
replace rhusb_edu=edu if male==1
bysort myhhid: egen wife_edu = mean(rwife_edu)
bysort myhhid: egen husb_edu = mean(rhusb_edu)

*keep only one observation per household
keep if tag==1

*create 20 groups of relative income within couples
foreach var in rel_earn {
gen `var'_20 =.
replace `var'_20 =	0	if `var' == 0
replace `var'_20 =	0.025	if `var' >	0	& `var'<=	0.05
replace `var'_20 =	0.075	if `var' >	0.05	& `var'<=	0.1
replace `var'_20 =	0.125	if `var' >	0.1	& `var'<=	0.15
replace `var'_20 =	0.175	if `var' >	0.15	& `var'<=	0.2
replace `var'_20 =	0.225	if `var' >	0.2	& `var'<=	0.25
replace `var'_20 =	0.275	if `var' >	0.25	& `var'<=	0.3
replace `var'_20 =	0.325	if `var' >	0.3	& `var'<=	0.35
replace `var'_20 =	0.375	if `var' >	0.35	& `var'<=	0.4
replace `var'_20 =	0.425	if `var' >	0.4	& `var'<=	0.45
replace `var'_20 =	0.475	if `var' >	0.45	& `var'<=	0.5
replace `var'_20 =	0.525	if `var' >	0.5	& `var'<=	0.55
replace `var'_20 =	0.575	if `var' >	0.55	& `var'<=	0.6
replace `var'_20 =	0.625	if `var' >	0.6	& `var'<=	0.65
replace `var'_20 =	0.675	if `var' >	0.65	& `var'<=	0.7
replace `var'_20 =	0.725	if `var' >	0.7	& `var'<=	0.75
replace `var'_20 =	0.775	if `var' >	0.75	& `var'<=	0.8
replace `var'_20 =	0.825	if `var' >	0.8	& `var'<=	0.85
replace `var'_20 =	0.875	if `var' >	0.85	& `var'<=	0.9
replace `var'_20 =	0.925	if `var' >	0.9	& `var'<=	0.95
replace `var'_20 =	0.975	if `var' >	0.95	& `var'<	1
replace `var'_20 =	1	if `var' == 1
}


* tag extreme values
gen extreme = 0
replace extreme = 1 if rel_earn==0 | rel_earn==1
...
count if rel_earn == 0.5
count if rel_earn~=.
count if rel_earn~=. & extreme~=1

log off

* produce graphs and the McCrary tests

*define education groups
gen all = 1
gen bothLow = 1 if husb_edu == 0 & wife_edu == 0
gen bothHi = 1 if husb_edu == 1 & wife_edu == 1
gen husbHi = 1 if husb_edu == 1 & wife_edu == 0
gen wifeHi = 1 if husb_edu == 0 & wife_edu == 1

save `outputDir'temp, replace
...
* count the number of observations in each histogram cell
...
* generate the graph
bysort rel_earn_20: egen count_rel_earn_20 = count(rel_earn_20)
drop if rel_earn_20==.
gen n=1
egen tot = sum(n)
collapse count_rel_earn_20 tot (min) min_rel_earn = rel_earn (max) max_rel_earn=rel_earn, by(rel_earn_20)
gen frac = count_rel_earn_20/tot



scatter frac rel_earn_20, mfcolor(none) || lowess frac rel_earn_20 if rel_earn_20<=0.5, lpattern(dash) || lowess frac rel_earn_20 if rel_earn_20>0.5, lpattern(dash) legend(off) xtitle("Share earned by the wife") ytitle("Fraction of couples") scheme(s2mono) xline(0.5) ysc(ra(0.015 0.105)) ylabel(0.02 0.04 0.06 0.08 0.1)
graph save `outputDir'graph_rel_earn_`var'_${data},replace
}

* add the mccrary test (for all groups, both with and without 0s and 1s)
...
* drop the extremes and generate estimates

foreach var in all bothLow bothHi husbHi wifeHi {
	clear
	use `outputDir'temp
	keep if `var' == 1
	keep if extreme==0
...
log on
display "`var'"
display "extremes excluded"
display "cutoff is 0.5"
capture drop Xj Yj r0 fhat se_fhat
DCdensity rel_earn, breakpoint(0.5) generate(Xj Yj r0 fhat se_fhat) nograph


display "`var'"
display "extremes excluded"
display "cutoff is 0.5001"
capture drop Xj Yj r0 fhat se_fhat
DCdensity rel_earn, breakpoint(0.50001) generate(Xj Yj r0 fhat se_fhat) nograph
...
* keep the extremes and generate only estimates without graphs
