...
*** BASIC INFO;
%let variable=sipp_birthdate; /* name of sas variable on input file to be imputed */
...
*** STRATIFICATION;
%let num_lists=4; /* number of stratification lists provided from finest to coarsest */
/* next: list of numerical, categorical variables to stratify data before regression/bootstrap */
%let stratifiers1=male linked_couple linked_mom linked_child hhh_flag;
%let stratifiers2=male linked_couple linked_mom linked_child;
%let stratifiers3=male linked_mom linked_child;
%let stratifiers4=linked_mom linked_child;
/*******
...
%let minimum_variable=minvar;
%let maximum_variable=maxvar;
...
minvar=mdy(1,1,1900);
maxvar=mdy(1,1,2000);
if linked_mom then do;
  minvar=mdy(1,1,1940);
  maxvar=mdy(1,1,1985);
end;
if linked_child then minvar=mdy(1,1,1990);
else maxvar=mdy(1,1,1992);
