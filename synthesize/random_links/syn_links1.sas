...
%macro link_spouses;
  ...
  *** A couple macros to use in the main loop over panels and states;
  ...
  data
    ...
    set implib.&impfile.&implicate. (where=(linked_couple=1));
  ...
  data outlib.syn_spouse_xwalk&implicate.;
    set temp2_xwalk_imp;
  run;
  proc means data=outlib.syn_spouse_xwalk&implicate.;
    var distance;
  run;
  data temp;
    set outlib.syn_spouse_xwalk&implicate. nobs=nobs;
...
%if &implicate.=1 %then %do;
  proc kde data=_origdist_;
    univar distfrommean / out=kde1 gridl=0 gridu=100 ng=51;
  run;
  proc kde data=temp2_xwalk_imp;
    univar distfrommean / out=kde2 gridl=0 gridu=100 ng=51;
  run;
  data kde;
    set kde1 (rename=(density=density1));
    set kde2 (rename=(density=density2));
    retain fmax 0;
    fmax=max(fmax,density1,density2);
    fmax=0.01*ceil(100*max(fmax,density1,density2));
  ...
  legend1 label=none position=(top right inside) mode=share value=("Original Links" "Synthetic Links") across=1;
  
  axis1 order=(0 to 50 by 5);
  axis2 order=(0 to &fmax. by &fstep.);
  title "Distance of Predicted Mean Spouse to Linked Spouse";
  proc gplot data=kde;
    plot density1*value density2*value
      /overlay haxis=axis1 vaxis=axis2 name="distfrommean" legend=legend1;
  run;
  quit;
%end;
