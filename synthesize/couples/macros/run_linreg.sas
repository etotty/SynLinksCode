...
%macro run_linreg;
  ...
  %include "macros/linreg_impute.sas";
    ...
    *** loop over strata in this cluster;
    %do i=1 %to &nstrata.;
      ...
      *** prepare this imputation stratum for modifying the cluster after imputation;
      ...
      %linreg_impute(stratum,mstratum,&curvar.,&transform_depvar.,&regressors.,&transform_list.,
        interactions=&interaction_list.,seed=&seed.,minvar=&min_imp_var.,maxvar=&max_imp_var.,suffix=);
      %let seed=&outseed.;
      %put *********** end the impute ***************;
      proc means data=stratum print n mean std min p5 p10 q1 median q3 p90 p95;
        var &curvar.;
        title1 "Estimation Set: &curvar.";
      run;
      proc means data=mstratum print n mean std min p5 p10 q1 median q3 p90 p95;
        var &curvar.;
